...
def ensure_cifar100_data(data_root: str = "./data/") -> None:
    """Download CIFAR-100 from a fast mirror if torchvision has not already extracted it."""
    root = Path(data_root)
    root.mkdir(parents=True, exist_ok=True)
    extracted = root / "cifar-100-python"
    tarball = root / CIFAR100_TGZ

    if extracted.is_dir():
        return
    if tarball.is_file() and _file_md5(tarball) == CIFAR100_MD5:
        return
                ...
                with urllib.request.urlopen(url, timeout=60) as response:
                    while True:
                        ...
                        if not chunk:
                            break
                        tmp.write(chunk)
            if _file_md5(tmp_path) != CIFAR100_MD5:
                raise ValueError(f"MD5 mismatch for download from {url}")
            tmp_path.replace(tarball)
            logging.info("CIFAR-100 ready at %s", tarball)
            return
        except Exception as exc:
            last_error = exc
            logging.warning("CIFAR-100 download failed from %s: %s", url, exc)
            if tmp_path is not None and tmp_path.exists():
                tmp_path.unlink(missing_ok=True)

    raise RuntimeError(f"Failed to download CIFAR-100 from all mirrors: {last_error}")
...
class Cifar100Task(Task):
    def __init__(
        ...
        self.data = Cifar100Dataset("train", lock, device=self._device)
        ...
        if rank > -1:
            ...
            self.data = splits[0]
        self._test_data = Cifar100Dataset("test", lock, device=self._device)
...
class Cifar100Dataset(PyTorchDataset):
    ...
    def __init__(
        ...
        with lock:
            ensure_cifar100_data(data_root)
...
def download(lock):
    ensure_cifar100_data()
    Cifar100Dataset("train", lock)
