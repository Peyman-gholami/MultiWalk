...
class CifarTask(Task):
    def __init__(
        ...
        self.data = CifarDataset("train", lock, device=self._device)
        ...
        if rank > -1:
            ...
            self.data = splits[0]
        self._test_data = CifarDataset("test", lock, device=self._device)
    ...
    def _create_model(self):
        if self._model_name == "ResNet20":
            ...
            raise ValueError(f"Unsupported model_name '{self._model_name}' for Cifar")
        return model
