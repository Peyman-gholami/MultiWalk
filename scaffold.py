...
class HUScaffold(Scaffold):
    def receive_from_clients(self, participating_clients, global_parameters):
        """Receive delta from participating clients separately"""
        client_updates = {}

        # Initiate all non-blocking receives for parameter differences
        delta_receive_info = []
        for client_rank in participating_clients:
            delta_buffer = torch.zeros_like(pack(global_parameters))
            delta_request = dist.irecv(tensor=delta_buffer, src=client_rank)
            delta_receive_info.append((delta_request, delta_buffer, client_rank))

        # Wait for all parameter difference receives
        for delta_request, delta_buffer, client_rank in delta_receive_info:
            delta_request.wait()
            client_updates[client_rank] = unpack(delta_buffer, [param.shape for param in global_parameters])
    ...
    def server_process(self, server_rank, shared_parameter_arrays, shared_state_arrays):
        ...
        while time.time() < training_end_time:
            ...
            for client_rank, updates in client_updates.items():
                for global_param, delta_i, global_c in zip(global_parameters, updates, global_control_variates):
                    global_param.data += - self.global_learning_rate * local_lr_reference * self.parent.tau / len(participating_clients) * (delta_i.to(communication_device) + global_c)
        ...
        dist.destroy_process_group()
        logging.info(f"[HUSCAFFOLD Server] Finished {current_round} rounds")


    def client_process(self, client_rank,):
        """HUSCAFFOLD Client process (client_rank > 0) - performs local training with control variates"""
        torch.manual_seed(self.parent.config["seed"] + client_rank)
        np.random.seed(self.parent.config["seed"] + client_rank)
        comm_device = 'cpu'
        training_device = torch.device(f'cuda:{self.parent.local_rank}' if torch.cuda.is_available() else 'cpu')
        ...
        while True:
            ...
            notification = torch.tensor(-1, dtype=torch.int32).to(comm_device)
            dist.recv(tensor=notification, src=0)
            logging.info(f"[HUSCAFFOLD Client {client_rank}] notification received!")
            if notification.item() == -10:  # End signal
                break
            elif notification.item() == 0:  # End training
                 dist.barrier()
                ...
                dist.recv(tensor=model_buffer, src=0)
                global_params = unpack(model_buffer, [p.shape for p in parameters])
                logging.info(f"[HUSCAFFOLD Client {client_rank}] received the model!")

                # Receive global control variates from server
                logging.info(f"[HUSCAFFOLD Client {client_rank}] receiveing the control variates!")
                control_variates_buffer = torch.zeros_like(pack(client_control_variates), device=comm_device) # Control variates have same shape as parameters
                dist.recv(tensor=control_variates_buffer, src=0)
                global_control_variates = unpack(control_variates_buffer, [cv.shape for cv in client_control_variates])
                logging.info(f"[HUSCAFFOLD Client {client_rank}] received the control variates!")
                ...
                # Calculate parameter difference
                delta = [(global_param - param) / local_lr_reference / self.parent.tau - global_c for param, global_param, global_c in zip(parameters, global_params, global_control_variates)]

                # Calculate client control variate update

                client_control_variates = [client_control_variate + delta_i for client_control_variate, delta_i in zip(client_control_variates, delta)]

                # Send delta to server
                event_logger.log_start("communication")
                delta_buffer = pack(delta).to(comm_device)
                dist.send(tensor=delta_buffer, dst=0)
        ...
        dist.barrier()
        dist.destroy_process_group()
        logging.info(f"[HUSCAFFOLD Client {client_rank}] Finished")
...
class HScaffold(Scaffold):
    def send_to_clients(self, participating_clients, global_parameters, device, logger, round_number, server_rank):
        ...
        for notification_request in notification_requests:
            notification_request.wait()
        logging.info(f"[HSCAFFOLD Server] Round {round_number}, all notifications sent!")

        # Send global model to participating clients
        model_send_requests = []
        for client_rank in participating_clients:
            logging.info(f"[HSCAFFOLD Server] Round {round_number}, model is to be sent to rank {client_rank}!")
            logger.log_start("communication")
            model_buffer = pack(global_parameters)
            model_send_requests.append(dist.isend(tensor=model_buffer, dst=client_rank))
            bytes_transmitted = num_bytes(model_buffer)
            logger.log_end("communication", {"round": round_number, "from": server_rank, "to": client_rank, "bytes_sent": bytes_transmitted})
            logging.info(f"[HSCAFFOLD Server] Round {round_number}, model was sent to rank {client_rank}!")
    ...
    def receive_from_clients(self, participating_clients, global_parameters):
        """Receive updates (parameter differences) from participating clients separately"""
        client_updates = {}
        # Initiate all non-blocking receives for parameter differences
        param_diff_receive_info = []
        for client_rank in participating_clients:
    ...
    def server_process(self, server_rank, shared_parameter_arrays, shared_state_arrays):
        ...
        dist.destroy_process_group()
        logging.info(f"[HSCAFFOLD Server] Finished {current_round} rounds")


    def client_process(self, client_rank,):
        """HSCAFFOLD Client process (client_rank > 0) - performs local training with control variates"""
        torch.manual_seed(self.parent.config["seed"] + client_rank)
        np.random.seed(self.parent.config["seed"] + client_rank)
        comm_device = 'cpu'
        training_device = torch.device(f'cuda:{self.parent.local_rank}' if torch.cuda.is_available() else 'cpu')
        ...
        while True:
            ...
            notification = torch.tensor(-1, dtype=torch.int32).to(comm_device)
            dist.recv(tensor=notification, src=0)
            logging.info(f"[HSCAFFOLD Client {client_rank}] notification received!")
            if notification.item() == -10:  # End signal
                break
            elif notification.item() == 0:  # End training
                 dist.barrier()
                ...
                dist.recv(tensor=model_buffer, src=0)
                global_params = unpack(model_buffer, [p.shape for p in parameters])
                logging.info(f"[HSCAFFOLD Client {client_rank}] received the model!")
                ...
                global_control_variates = [(last_global_param - global_param) * int(self.participation_rate * (parent.size -1)) / (parent.size -1) / ( self.parent.tau * self.global_learning_rate * local_lr_reference )  for global_param, last_global_param in zip(global_params, last_global_params)]
